...
function adicionarLinha() {
  ...
  if (!semana || !objetivo || !status) {
      alert('Por favor, preencha todos os campos antes de adicionar um novo dado.');
      return;
  }
...
function adicionarLinha2() {
  ...
  cell3.innerHTML = valorFormatado;
...
function editarLinha2(botao) {
  ...
  var valorSemSimbolo = tabela.rows[index].cells[2].innerHTML.replace('R$ ', '');
  document.getElementById('valor').value = valorSemSimbolo;
...
function ocultarTodosConteineres() {
  ...
  var fotoContainer = document.getElementById('fotoOdonto');
...
// Função para mostrar o projeto final 
function foto() {
  ocultarTodosConteineres();
  var fotoOdonto = document.getElementById('fotoOdonto');
  fotoOdonto.style.display = fotoOdonto.style.display === 'none' ? 'block' : 'none';
}